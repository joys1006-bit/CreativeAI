...
class AuthController {
    ...
    fun signup(@RequestBody request: SignupRequest): Mono<ApiResponse<AuthTokenResponse>> {
        ...
        if (request.email.isBlank()) {
            return Mono.just(ApiResponse(success = false, data = null, message = "이메일은 필수입니다."))
        }
        if (request.password.length < 6) {
            return Mono.just(
                    ApiResponse(success = false, data = null, message = "비밀번호는 6자 이상이어야 합니다.")
            )
        }
        if (request.username.isBlank()) {
            return Mono.just(ApiResponse(success = false, data = null, message = "사용자명은 필수입니다."))
        }
    ...
    fun login(
        ...
        return authService
                ...
                .map { tokenResponse ->
                    ApiResponse(success = true, data = tokenResponse, message = "로그인되었습니다.")
                }
                .onErrorResume { error ->
                    Mono.just(
                            ApiResponse(
    ...
    fun refreshToken(
        ...
        return authService
                ...
                .map { tokenResponse ->
                    ApiResponse(success = true, data = tokenResponse, message = "토큰이 갱신되었습니다.")
                }
                .onErrorResume { error ->
                    Mono.just(
                            ApiResponse(
    ...
    fun getCurrentUser(
        ...
        return authService
                ...
                .map { userInfo ->
                    ApiResponse(success = true, data = userInfo, message = "사용자 정보 조회 성공")
                }
                .onErrorResume { error ->
                    Mono.just(
                            ApiResponse(
