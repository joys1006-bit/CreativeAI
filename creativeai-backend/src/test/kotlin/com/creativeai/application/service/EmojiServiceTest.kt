...
class EmojiServiceTest {
    ...
    fun `이모티콘 생성 요청이 성공하면 PROCESSING 상태를 반환한다`() {
        ...
        every { aiModelPort.generateEmoji(any(), any()) } returns Mono.just(
            GeneratedEmojiData("generated", listOf("var1", "var2"))
        )
    ...
    fun `스타일 목록 조회가 성공한다`() {
        // when
        val result = emojiService.execute()
        
        // then
        StepVerifier.create(result)
            .assertNext { styles ->
                assertEquals(6, styles.size)
                assertEquals("kakao", styles[0].id)
            }
            .verifyComplete()
    }
}
