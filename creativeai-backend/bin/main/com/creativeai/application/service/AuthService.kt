...
class AuthService {
    ...
    fun login(
        ...
        return userRepository
                ...
                .switchIfEmpty(Mono.error(IllegalArgumentException("이메일 또는 비밀번호가 올바르지 않습니다.")))
                .flatMap { user ->
                    // 비밀번호 확인
    ...
    fun refreshTokens(refreshToken: String): Mono<AuthTokenResponse> {
        ...
        return refreshTokenRepository
                ...
                .flatMap { storedToken ->
                    if (storedToken.isRevoked || storedToken.expiresAt.isBefore(LocalDateTime.now())
                    ) {
                        ...
                        userRepository
                                ...
                                .flatMap { existingUser ->
                                    ...
                                    saveOAuthAccount(
                                            ...
                                            .flatMap { userRepository.findById(existingUser.id) }
                                            .flatMap { user -> generateTokens(user!!) }
                                }
                                .switchIfEmpty(
                                        // 3. 완전히 새로운 사용자 생성
                                        Mono.defer {
                                            ...
                                            userRepository.save(newUser).flatMap { savedUser ->
                                                saveOAuthAccount(
                                                        ...
                                                        .flatMap { generateTokens(savedUser) }
                                            }
                                        }
                                )
                )
