...
### 8. AI 모델 업그레이드 시 반드시 A/B 비교 테스트를 먼저 하라
- **사건**: Whisper `base`→`small` 모델 변경 + 파라미터 변경 → 한국어 싱크 오히려 악화
- **원인**: 검증 없이 "더 큰 모델이 더 좋을 것"이라고 가정하고 적용
- **교훈**: **AI 모델 변경 시 반드시 동일 데이터로 기존 vs 신규 A/B 비교 후 적용. 특히 언어별(한국어) 특성에 따라 결과가 달라질 수 있음**
